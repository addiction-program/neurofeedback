%%
%Last edited by Amir Dakhili on September 24th, 2024
%%
...
feedback_dir = 'Z:\TBVData\HumanData\NFB\NFB\'; % Path to the feedback folder
...
pp_no = 1;
pp_name = 'Ethan';
num_blocks = 2; % Number of times to repeat the craving task and Likert scale
...
%     full run  %write number of volumes, runs, total time 
     % blockOrder = { 'R', 'V', 'F', 'U', 'F', 'N', 'F','U', 'F', 'N', 'F','U', 'F', 'N', 'F','U', 'F', 'N', 'F','U', 'F', 'N', 'F', 'V', 'F', 'D', 'F', 'N', 'F','D', 'F', 'N', 'F','D', 'F', 'N', 'F','D', 'F', 'N', 'F','D', 'F', 'N', 'F', 'V'};
   ...
   % Start of the experiment loop
    for block_num = 1:num_blocks
        ...
        % Variables to store first rest and last VAS timings for each block set
        firstRestStartTime = NaN;  % Initialize with NaN
        lastVASEndTime = NaN;     % Initialize with NaN
            ...
            switch currentBlock
                ...
                case 'R' 
                    if run_rest_block
                        ...
                        % Capture the first rest block start time
                        if isnan(firstRestStartTime)
                            firstRestStartTime = block_start;
                        end
                    end 
                ...
                case 'U'
                    if run_upregulation_block
                        ...
                        [block_start, block_end, block_dur, block_start_TR, block_end_TR, block_start_TBV_TR, block_end_TBV_TR, image_onsets, image_durations, image_offsets, cravingImageNumbers, selectedImageIndices,allSelectedImages] = Upregulation_feedback(...
                            block_dur_TR, feedback_dir, feedback_file_name, imageTextures, p, selectedImageIndices, allSelectedImages);
                ...
                case 'D'
                    if run_downregulation_block
                        if p == find(strcmp(blockOrder, 'D'), 1) && block_num == 1
                            ...
                            % Downregulation CUE
                            Text = 'While you view five images, \n try to decrease your craving for the drug.';
                            [cue_start, ~, ~, cue_start_TR, ~, cue_start_TBV_TR, ~] = WriteInstruction(Text, magenta, cue_dur_TR, feedback_dir, feedback_file_name);
...
%                         BlankOut(1, feedback_dir, feedback_file_name);
                        ...
                        [block_start, block_end, block_dur, block_start_TR, block_end_TR, block_start_TBV_TR, block_end_TBV_TR] = likert_scale(window, scr_rect, instructionText, feedback_dir, feedback_file_name, fileID7, p, num_blocks, block_num, blockOrder);
                    ...
                    % Update the last VAS block end time
                    lastVASEndTime = block_end; 
...
%                         BlankOut(1, feedback_dir, feedback_file_name);
        ...
        % Write the first rest start time and last VAS end time to fileID2 (formatted)

    fprintf(fileID2, '%.2f  %.2f  %.2f  %d  %d  %d\n', firstRestStartTime, lastVASEndTime, lastVASEndTime - firstRestStartTime, ...
            round(firstRestStartTime / TR) + 1, round(lastVASEndTime / TR), round(lastVASEndTime / TR) - round(firstRestStartTime / TR) + 1);
    ...
    Total_run_duration = VAS_block_timings(end, 2) - rest_block_timings(1, 1); %secs
...
function [bo, be, bdur, block_start_tr, block_end_tr, block_start_tbv_tr, block_end_tbv_tr, image_onsets, image_durations, image_offsets, cravingImageNumbers, selectedImageIndices,allSelectedImages] = Upregulation_feedback(num_trs, folder_path, file_prefix, imageTextures, p, selectedImageIndices, allSelectedImages)  
...
function [bo, be, bdur, block_start_tr, block_end_tr, block_start_tbv_tr, block_end_tbv_tr, image_onsets, image_durations, image_offsets, cravingImageNumbers, selectedImageIndices,allSelectedImages] = Downregulation_feedback(num_trs, folder_path, file_prefix, imageTextures, p, selectedImageIndices, allSelectedImages)  
    global window start_time current_TBV_tr TR windowHeight FB_timings 
...
function [bo, be, bdur, block_start_tr, block_end_tr, block_start_tbv_tr, block_end_tbv_tr] = Neutral_control(num_trs, folder_path, file_prefix, imageDir, cravingImageNumbers, p)
    ...
    if isempty(cravingImageNumbers)  % If cravingImageNumbers is empty (neutral block first)
        ...
        if isempty(selectedNeutralImageIndices)
            selectedNeutralImageIndices = randperm(numNeutralImages, 5);
        else
            remainingNeutralImageIndices = setdiff(allNeutralImageIndices, selectedNeutralImageIndices);
            selectedNeutralImageIndices = remainingNeutralImageIndices(randperm(numel(remainingNeutralImageIndices), 5));
        end
...
function DrawNeutralFeedback()
    ...
    barOffsetX = x_size * 5;     
    barOffsetY = 0;    
    all_rect_coords = zeros(4, 40);  % Adjusted for 40 rectangles
    rect_start_pos = -20;  % Adjust for 40 rectangles
    ...
    Screen('FrameRect', window, 255, all_rect_coords, ones(40, 1) * 1.5);
...
function rect_num = DrawFeedback(score)
    ...
    all_rect_coords = zeros(4,40);  % Adjusted for 40 rectangles
    rect_start_pos = -20;  % Adjust for positioning 40 rectangles
    for i = 1:40
    ...
    if score > 0
        ...
        Screen('FillRect', window, repmat(rect_color', [1, score]), all_rect_coords(:, 21:(20+score)));
    end
...
function rect_num = DrawFeedbackDownregulation(score)  
    ...
    all_rect_coords = zeros(4, 40);  % Adjusted for 40 rectangles
    rect_start_pos = -20;  % Start position adjusted for 40 rectangles
...
%%%%%%%%%%%%%%%%%%%%%%Likert Function%%%%%%%%%%%%%%
function [block_start, block_end, block_dur, block_start_TR, block_end_TR, block_start_TBV_TR, block_end_TBV_TR, rating] = likert_scale(window, scr_rect, instructionText, feedback_dir, feedback_file_name, fileID7, p, num_blocks, block_num, blockOrder) 
    ...
    % Check if it's the last VAS of the last block OR the last VAS within any block except the last block
    isLastVASOfLastBlock = (block_num == num_blocks) * (p == length(blockOrder));
    isLastVASOfIntermediateBlock = (p == length(blockOrder)) * (block_num < num_blocks); 
    fprintf('VAS_num: %d, block_num: %d, i: %d, num_blocks: %d\n', p, block_num, p, num_blocks);

    if isLastVASOfLastBlock  % If it's the very last VAS of the experiment
        % Get the starting time of the block
        disp('Identified the last VAS of the last block');
        ...
        % Store the rating in the provided file (include VAS_num)
        fprintf(fileID7, '%d        %.2f        %d\n', p, block_end, rating);

%     elseif isLastVASOfIntermediateBlock  % If it's the last VAS of any block before the last one, skip it
%         disp('Identified the last VAS of an intermediate block, skipping...');
%         % Assign default values to output arguments
%         block_start = NaN;  % Or any other appropriate default value
%         block_end = NaN;
%         block_dur = NaN;
%         block_start_TR = NaN;
%         block_end_TR = NaN;
%         block_start_TBV_TR = NaN;
%         block_end_TBV_TR = NaN;
%         rating = NaN;
%         
    elseif isLastVASOfIntermediateBlock
        disp('Presenting normal VAS block');
        % Get the starting time of the block
        block_start = GetSecs() - start_time;
        block_start_TR = round(block_start / TR) + 1;
        block_start_TBV_TR = current_TBV_tr;
    ...
    else  % For all other VAS blocks, present them normally
        disp('Presenting normal VAS block');
        % Get the starting time of the block
        block_start = GetSecs() - start_time;
        block_start_TR = round(block_start / TR) + 1;

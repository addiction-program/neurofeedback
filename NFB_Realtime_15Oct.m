...
function [bo, be, bdur, block_start_tr, block_end_tr, block_start_tbv_tr, block_end_tbv_tr] = Neutral_control(num_trs, folder_path, file_prefix, imageDir, cravingImageNumbers, p)
    ...
    if isempty(cravingImageNumbers)  % If cravingImageNumbers is empty (neutral block first)
        ...
        if isempty(selectedNeutralImageIndices)
            selectedNeutralImageIndices = randperm(numNeutralImages, 3);
        else
...
function [block_start, block_end, block_dur, block_start_TR, block_end_TR, block_start_TBV_TR, block_end_TBV_TR, rating] = VAS_scale(window, scr_rect, instructionText, feedback_dir, feedback_file_name, fileID7, p, num_blocks, block_num, blockOrder) 
    ...
    if isLastVASOfLastBlock  % If it's the very last VAS of the experiment
        ...
        % Store the rating in the provided file 
        if block_num == 1 && strcmp(blockOrder{p}, 'PR')  % Check for the first VAS block (practice)
            fprintf(fileID7, '%d        %.2f        %d        Practice\n', p, block_end, rating); 
        else  % For all other VAS blocks
